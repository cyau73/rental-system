...
export default async function AdminPage({
  ...
  // 2. Data Fetching with Filter Logic
    ...
    <div className="min-h-screen bg-gray-50 font-sans selection:bg-blue-100">
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-6 lg:p-10">
            ...
            <form action={addProperty} className="flex flex-col gap-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="flex flex-col gap-1">
                  ...
                  <input name="title" placeholder="Villa Name" className="border border-gray-200 p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm transition-all" required />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[9px] font-bold text-gray-400 ml-1 uppercase tracking-widest">Location</label>
                  <input name="address" placeholder="Address" className="border border-gray-200 p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm transition-all" required />
                </div>
                <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-4">
          {properties.length > 0 ? (
            properties.map((prop) => {
              // Ensure URL works even if base URL is wrong in .env
              const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || '';
              const shareUrl = `${baseUrl}/properties/${prop.id}`;
              const whatsappLink = `https://wa.me/?text=${encodeURIComponent(`Check out this property: ${prop.title} - ${shareUrl}`)}`;
            ...
            <div className="bg-white rounded-[2rem] border-2 border-dashed border-gray-200 py-20 text-center text-gray-400 text-xs italic">No properties found.</div>
          )}
        </div>
      </main>
      <ScrollToTop />
